...
export function DeliverAttributeData(gl,name,vertexData,size,offset = 0){
  let vertexTexCoordBuffer = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, vertexTexCoordBuffer);
  gl.bufferData(gl.ARRAY_BUFFER, vertexData, gl.STATIC_DRAW);
  let FSIZE = vertexData.BYTES_PER_ELEMENT;
  let a_Position = gl.getAttribLocation(gl.program, name);
  gl.vertexAttribPointer(a_Position, size, gl.FLOAT, false, FSIZE * size, FSIZE*offset);
