...
const c = new THREE.Vector3(-50, 0, 0);//与a平行且方向相反

const aMesh = SphereCreate(a,0x00ffff)
const bMesh = SphereCreate(b,0xff0000)
const cMesh = SphereCreate(c,0x0000ff)

const abhelper = new THREE.ArrowHelper(b.clone().sub(a).normalize(),a,b.clone().sub(a).length())
scene.add(abhelper)
