...
const p2 = new THREE.Vector3(20,15,0);// 点2坐标
...
//计算p1->p2,p1->p3这两条线的夹角
const a = p2.clone().sub(p1)
const b = p3.clone().sub(p1)
const angle = CalculateAngle(a,b)
console.log(`p1点的夹角为${angle}`)

//计算p2->p1,p2->p3这两条线的夹角
const c = p1.clone().sub(p2)
const d = p3.clone().sub(p2)
const angle1 = CalculateAngle(c,d)
console.log(`p2点的夹角为${angle1}`)

//计算p3->p1,p3->p2这两条线的夹角
const e = p1.clone().sub(p3)
const f = p2.clone().sub(p3)
const angle2 = CalculateAngle(e,f)
console.log(`p3点的夹角为${angle2}`)
