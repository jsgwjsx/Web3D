...
const vec = p1.clone().sub(p2)

const c = p3.clone().cross(vec)

const ahelper = new THREE.ArrowHelper(vec.clone().normalize(),p2,vec.length(),0x00ff00)
const bhelper = new THREE.ArrowHelper(p3.clone().normalize(),p2,20,0xff0000)
const chelper = new THREE.ArrowHelper(c.clone().normalize(),p2,c.length(),0x0000ff)
scene.add(ahelper,bhelper,chelper)
