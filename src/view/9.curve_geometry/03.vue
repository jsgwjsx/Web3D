...
//创建一个EllipseCurve椭圆曲线对象(椭圆的中心的X坐标,椭圆的中心的y坐标, X轴向上椭圆的半径, y轴向上椭圆的半径，开始的角度，结束的角度,是否顺时针绘制)
//const ellipse = new THREE.EllipseCurve(0,0,100,50)绘制一个椭圆
//圆弧绘制一个ArcCurve圆弧对象(圆的中心的X坐标,圆的中心的y坐标, 圆的半径半径，开始的角度，结束的角度,是否顺时针绘制)
const arc = new THREE.ArcCurve(0,0,50,0,Math.PI/3*2,false)

//获取点的坐标
//const pointsArr = ellipse.getPoints(50)//获取51个点,返回的都是矢量对象，可以通过setFromPoints传给Buffergeometry.attribute.position属性
const pointsArr = arc.getSpacedPoints(50)//等间距划分
...
const point = new THREE.Line(geometry1,material1) 
scene.add(point)
