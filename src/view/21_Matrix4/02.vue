...
//通过vec3对线.applyMatrix4()来对向量进行改变
...
mesh.updateMatrix() //更新模型的矩阵，.matrix会发生变化,renderer.render时也会自动更新
mesh.updateMatrixWorld()//更新模型的世界矩阵，.matrixWorld会变化,renderer.render时也会自动更新
console.log('本地矩阵',mesh.matrix);
console.log('世界矩阵',mesh.matrixWorld);//世界矩阵属性.matrixWorld是自身及其所有父对象本地矩阵属性.matrix的复合矩阵。
console.log('本地矩阵',group.matrix);
console.log('世界矩阵',group.matrixWorld);
scene.add(group)
...
function initProgram(canvasid) {
    ...
    function render() {
        //renderer.render(scene, camera)
        //renderer.render(scene,camera)后，会自动更新模型的本地矩阵与世界矩阵
