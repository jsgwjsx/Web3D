<template>
    <div>
        <div id="map"></div>
    </div>
</template>
<script setup>
import * as Cesium from "cesium";
import { onMounted } from 'vue'
onMounted(() => {
    Cesium.Ion.defaultAccessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiI5OTNhNzM4Zi05OGM1LTQzNzgtOWY3OC1mMjkyMDRjNGQ2NWIiLCJpZCI6MjIwMDczLCJpYXQiOjE3MjQ5ODE0OTN9.xeCfpceKEj1anyoP4fLDosWa-0gNwB1fm-IDE7-uplc'
    var viewer = new Cesium.Viewer('map', {
        //搜索控件
        geocoder: false,
        //home控件
        homeButton: false,
        //动画控件
        animation: false,
        //全屏控件
        fullscreenButton: false,
        //场景模式选择器
        sceneModePicker: false,
        //时间轴
        timeline: false,
        //导航帮助按钮
        navigationHelpButton: false,
        //底图选择器
        baseLayerPicker: false,
        //1. 通过baseLayer在viewer内部中添加地图
        baseLayer: new Cesium.ImageryLayer(new Cesium.UrlTemplateImageryProvider({
            url: ' http://webrd01.is.autonavi.com/appmaptile?&scale=1&lang=zh_cn&style=8&x={x}&y={y}&z={z}',
            minimumLevel: 1,
            maximumLevel: 18
        })
        )
    })
    viewer.scene.debugShowFramesPerSecond = true //显示fps
    const entity = viewer.entities.add({
        label: {
            show: false,
            showBackground: true,
            font: "14px monospace",
            horizontalOrigin: Cesium.HorizontalOrigin.LEFT,
            verticalOrigin: Cesium.VerticalOrigin.TOP,
            pixelOffset: new Cesium.Cartesian2(15, 0),
        }
    })
    var handler = new Cesium.ScreenSpaceEventHandler(viewer.scene.canvas)
    handler.setInputAction(function (movement) {
        //1.创造光线
        var ray = viewer.camera.getPickRay(movement.position)//如果是MOUSE_MOVE事件，仅仅使用endP0sition坐标即可
        //2.查看广西是否与地球相交以及相交位置(世界坐标)
        var position = viewer.scene.globe.pick(ray, viewer.scene)
        if (position) {
            var lonlat = Cesium.Cartographic.fromCartesian(position)
            var latitude = Cesium.Math.toDegrees(
                lonlat.latitude
            )
            var longitude = Cesium.Math.toDegrees(
                lonlat.longitude
            )
            entity.position = position
            entity.label.show = true;
            entity.label.text =
                `Lon:   ${longitude}` +
                `\nLat:   ${latitude}`;
        }

    }, Cesium.ScreenSpaceEventType.LEFT_CLICK)
    const ray = new Cesium.Ray(new Cesium.Cartesian3.fromDegrees(100, 20, 300), new Cesium.Cartesian3(10, 0, 0))
    const ellipsoid = viewer.scene.globe.ellipsoid
    const Interval = Cesium.IntersectionTests.rayEllipsoid(ray, ellipsoid)
    console.log(Interval);
})
</script>
<style scoped>
#map {
    width: 100%;
    height: 100%;
}

* {
    margin: 0;
    padding: 0;
}

html,
body {
    width: 100%;
    height: 100%;
}
</style>