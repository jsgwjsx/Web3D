...
onMounted(() => {
    ...
    console.log(new Date().setTime(new Date(stoptime) - 2000))
    //设置地图的结束时间
    viewer.clock.stopTime = Cesium.JulianDate.fromDate(new Date(new Date().setTime(new Date(stoptime) - 500)));
    ...
    //创建锥形
    function addCylinder(lon, lat) {
        //创建一堆圆锥
        const cylinders = new Cesium.CylinderGeometry({
            length: 40,
            topRadius: 10,
            bottomRadius: 0,
        });
        const geometrys = Cesium.CylinderGeometry.createGeometry(cylinders);
        const instance = new Cesium.GeometryInstance({
            geometry: geometrys,
            modelMatrix: Cesium.Transforms.eastNorthUpToFixedFrame(Cesium.Cartesian3.fromDegrees(lon, lat, 20)),
    ...
    let Cyliders = []
    for (let i = 117.45; i < 117.46; i += 0.001) {
        for (let j = 31.14; j < 31.15; j += 0.001) {
            var primitive = addCylinder(i, j)
            Cyliders.push({ cylinder:primitive, position: Cesium.Cartesian3.fromDegrees(i, j, 4000), m: null })

        }
    }
    viewer.camera.setView({
        destination: Cyliders[0].position
    })
    //时间轴的监听回调函数
    ...
    viewer.clock.onTick.addEventListener((tick) => {
        ...
        //计算车与圆锥的夹角，来影响圆锥的姿态角度
    })
    viewer.scene.preUpdate.addEventListener(() => {
        //获取当前小车位置
        let modelposition = entitydd.position.getValue(viewer.clock.currentTime)
        if(!modelposition){
            return 
        }
        for (let i = 0; i < Cyliders.length; i++) {
            let cylinder = Cyliders[i]
            //获取圆锥中心
